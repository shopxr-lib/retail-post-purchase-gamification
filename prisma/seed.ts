...
async function main() {
  ...
  //         },
  //         {
  //           name: "Jackpot on 10th Play",
  //           type: 3,
  //           value: "$200 Shopping Spree",
  //           winningProbabilityMode: ProbabilityMode.CREDIT_COUNT,
  //           winningProbabilityValue: 10,
  //         },
  //         {
  ...
  for (const prize of prizes) {
    const exists = await db.prize.findFirst({
      where: {
        campaignId: campaign.id,
        name: prize.name,
      },
    ...
    if (!exists) {
      ...
      console.log(`✅ Added ${prize.name}`);
    } else {
      console.log(`⏭️ ${prize.name} already exists`);
    }
  }
}
